...
function PlanAndBillingPage() {
  ...
  const plans = [
    { id: 'Normal', name: 'Normal', validEmails: 1050, verifications: 2050, monthlyPrice: 16 },
    { id: 'Standard', name: 'Standard', validEmails: 6000, verifications: 10050, monthlyPrice: 69, popular: true },
    { id: 'Premium', name: 'Premium', validEmails: 20050, verifications: 40050, monthlyPrice: 149 },
  ];

  const getPrice = (monthlyPrice) => (isAnnual ? monthlyPrice * 12 : monthlyPrice);
  const getUpdatedValue = (value) => (isAnnual ? value * 12 : value);
  ...
  return (
    <div className="py-4 px-3 text-gray-90">
      <div className="max-w-5xl mx-auto flex justify-between items-start">
        ...
        <div className="w-1/3 lg:translate-y-11 lg:-translate-x-20">
        ...
        {/* Right Side - Plans */}
        <div className="w-[1400px] grid grid-cols-1 sm:grid-cols-3 gap-4">
          {plans.map((plan) => (
            <div
              key={plan.id}
            ...
            >
              ...
              <p className="text-xs mb-1"><span className="font-medium">{formatNumber(getUpdatedValue(plan.validEmails))}</span> emails</p>
              <p className="text-xs mb-2"><span className="font-medium">{formatNumber(getUpdatedValue(plan.verifications))}</span> verifications</p>
              ...
              <button
                ...
                className={`w-full py-1 px-2 text-sm rounded-md font-medium transition-all duration-300 ${plan.id === selectedPlan ? 'bg-blue-600 text-white' : 'border border-gray-200'}`}
              >
